#####################################################################################
# $Id: 22_HOMEMODE.pm 14135 2017-04-29 10:15:49Z DeeSPe $
#
# Usage
# 
...
sub HOMEMODE_Notify($$)
{
  ...
  if ($hash->{SENSORSLUMINANCE} && grep(/^$devname$/,split /,/,$hash->{SENSORSLUMINANCE}))
  {
    ...
    if (grep(/^$read:\s.*$/,@{$events}))
    {
      ...
      HOMEMODE_Luminance($hash,$devname,$lum);
    }
  }
  ...
  if ($hash->{SENSORSENERGY} && grep(/^$devname$/,split /,/,$hash->{SENSORSENERGY}))
  {
    ...
    foreach my $evt (@{$events})
    {
      ...
      {
        my $val = (split " ",$2)[0];
        HOMEMODE_PowerEnergy($hash,$devname,$1,$val);
      }
    }
  }
...
sub HOMEMODE_updateInternals($;$)
{
  ...
  else
  {
    ...
    if ($holiday)
    {
      foreach my $h (devspec2array($holiday))
      {
        push @allMonitoredDevices,$h if (!grep(/^$h$/,@allMonitoredDevices));
      }
    }
  ...
  if ($cmd eq "mode")
  {
    ...
    elsif ($option =~ /^(absent|gone)$/)
    {
      ...
      if ($attr{$name}{HomeModeAbsentBelatedTime} && $attr{$name}{"HomeCMDmode-absent-belated"})
      {
        ...
        CommandDefine(undef,"-temporary atTmp_absent_belated_$name at +$hour {HOMEMODE_execCMDs_belated(\"$name\",\"HomeCMDmode-absent-belated\",\"$option\")}");
      }
    }
